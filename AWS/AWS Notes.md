...
- Vertical Scalability means, in terms of EC2, means being able to increase (or decrease) the size of you instance, for example, increasing from a t2.micro to a t2.large. This is very common for non-distributed systems such as databases. 
- Horizontal Scalability, in terms of EC2, means being able to increase (or decrease) the number of systems / instances for your application, this impies distributed systems. This is very common for web applications. You can use auto scaling groups or load balancers to do this. 
- High availability, in terms of EC2, means that you are running your application/system in at least two AZs. The goal of this is to survive a data center loss or a disaster. 
- Scalability Definition: Ability to accommodate a larger load by making the hardware stronger, or by adding nodes.
- Elasticity Definition: Once a system is scalable, elasticity means that there will be some auto scaling so that the system can scale based on the load it receives. This is cloud-friendly and results in pay-per-use, matching demand, and optimizing costs.
- Agility Definition: Not directly related to scalability or elasticity, it means that the new IT resources are only a click away and greatly reduces the time it takes to make those resources availble to your developers from weeks to minutes.
- Load balancers are servers that forward internet traffic to multiple servers (EC2 instances) downstream. They allow you to expose a single point of access (DNS) to your app, Seamlessly handle failures, do regular health checks, provide ssl termination (HTTPS) for you websites, etc.
- Elastic Load Balancer (ELB) is a managed load balancer from AWS. AWS guarantees that it will always be working, takes care of upgrades, etc. You can set up your own, it costs less, but requires a lot more effort. 
- There are 4 kinds of load balancers provided by AWS: 1) Application Load Balancer (HTTP/HTTPS only and is a layer 7 type) 2) Network Load Balancer (has ultra-high performance, allows for TCP, and is a layer 4 type) 3) Gateway load balancer (layer 3 type) 4) Classic load balancer (Layer 4 type, but retired in 2023).
- Application Load Balancer: HTTP/HTTPS protocols, layer 7. Has HTTP routing features and offers a static DNS (url). 
- Network Load Balancer: TCP/UDP protocols, layer 4. Ultra High Performance (millions of requests per second). Offers a static IP through elastic IP.
- Gateway Load Balancer: GENEVE protocol on IP packets, layer 3. This routes traffic to firewalls that you manage on EC2 Instances so that you can do intrusion detection or deep packet inspection.
- How to create an ALB: Go to Load Balancers > create load balancer > select ALB. Name it, then select all AZs, add a security group (create a new one to allow all HTTP, if necessary), then create a target group to group our 2 instances together. When creating target group: name it, select instances, keep protocol as http on port 80, keep everything else as is and create it, then register the targets (instances). then in alb creation, select the target group. Once it spins up, you can access the target group by pasting the dns name into a browser.
- Auto Scaling Group: the goal of an ASG is to automatically scale out and in to match load, ensure we have the minimum and maximum number of machines running, automatically register new instances to a load balancer, and replace unhealthy instances, etc. Huge cost savings with this because your app only runs at optimal capacity.
- How to create an auto scaling group: Click Auto Scaling Groups section > create > name it > then create a launch template. Name the launch template (this tells the ASG how to create EC2 instances within it) > select the OS > instance type > everything else very similar to creating an instance. Once it is created, select the template in the ASG creation section > next > choose the different AZs > next > attach to existing load balancer > turn on health checks > enter desired capacity min capacity and max capacity. Keep everything else the same. Once created, update health checks to have a minimum healthy threshold of 2 and a timeout of 2, etc... Now, if you terminate an instance, one will be auto created since a minimum capacity is 2, pretty cool.
- ASG - Implement Elasticity in your app. Scaling Strategies: 1) Manual Scaling, updating the size of an ASG manually 2) Dynamic Scaling, responds automatically to changing demand. Here you can have simple/step scaling, example is when something, x, happens, add n units. You also have target tracking scaling which is where you tell the ASG to make sure CPU usage always is around 40%, for example. Also you have scheduled scaling, example is FRI at 5pm inc min capacity to 5 instances. 3) predictive scaling, this uses ML to predict future traffic ahead of time using historical traffic patterns and provisions the right num of instances automatically. This is useful for predictive time based patterns. 