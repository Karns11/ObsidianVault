...
- EC2 Instance Connect is an alternative to sshing into the instance from your command line. It does use SSH under the hood, so keep in mind that security group rule should be enabled for port 22. Note AWS Linux 2 AMI comes equipped with aws CLI which is really helpful and allows you to run aws iam commands for example.
